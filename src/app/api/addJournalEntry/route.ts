...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user === "Anonymous") {
      ...
      return new NextResponse(JSON.stringify({ message: "Journal entry submitted successfully!", posts: updatedPosts }), { status: 200 });
    }
    
    // Respond with a success message
    return new NextResponse(JSON.stringify({ message: "No journal entries done yet but we're working on it!" }), { status: 200 });
  } catch (error) {
    console.error("Error handling journal entry submission:", error);
    
    // Return an error response with status code 500
    return new NextResponse(
      JSON.stringify({ error: "An error occurred while submitting your journal entry." }),
      { status: 500 }
    );
  }
}